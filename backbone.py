...
class EfficientDetBackbone(nn.Module):
    def __init__(self, num_classes=80, compound_coef=0, load_weights=False, onnx_export=False, **kwargs):
        ...
        self.box_class_repeats = [1, 3, 3, 4, 4, 4, 5, 5, 5]
        self.pyramid_levels = [3, 5, 5, 5, 5, 5, 5, 5, 6]
    ...
    def forward(self, inputs):
        ...
        features = (p3, p4, p5)
        features = self.bifpn(features)[:3]  # [[1, 64, 64, 64], [1, 64, 32, 32], [1, 64, 16, 16], [1, 64, 8, 8], [1, 64, 4, 4]]
        # return features

        regression = self.regressor(features)  # 1, 49104, 4
        classification = self.classifier(features)  # [1, 49104, 90]
        anchors = self.anchors(inputs, inputs.dtype)  # 1, 49104, 4
